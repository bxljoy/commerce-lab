...
class OutboxDeliveryStoreIT extends AbstractPostgresIntegrationTest {
    ...
    @Test void onlyDueUndeliveredUnleasedRowsAreClaimed() {
        ...
        jdbc.update("UPDATE order_outbox SET next_attempt_at=clock_timestamp()+interval '1 hour' WHERE event_id=?",
                later.eventId());
        OutboxMessage delivered = seed();
        jdbc.update("UPDATE order_outbox SET delivered_at=clock_timestamp() WHERE event_id=?", delivered.eventId());
        ...
        assertThat(jdbc.queryForObject("SELECT lease_until > clock_timestamp() FROM order_outbox WHERE event_id=?",
    ...
    @Test void simultaneousClaimsHaveOnlyOneWinnerForOneRow() throws Exception {
        ...
        try (var workers = Executors.newFixedThreadPool(2)) {
            ...
            assertThat(jdbc.queryForObject("SELECT attempt_count FROM order_outbox", Long.class)).isEqualTo(1);
        }
    }
    ...
    @Test void skipsLockedCandidateWithoutWaitingForItsTransaction() throws Exception {
        ...
        try (var worker = Executors.newSingleThreadExecutor()) {
            new TransactionTemplate(transactionManager).executeWithoutResult(tx -> {
                jdbc.queryForObject("SELECT event_id FROM order_outbox WHERE event_id=? FOR UPDATE",
    ...
    @Test void expiredLeaseFixtureReclaimsAndRejectsBothStaleCompletionPaths() {
        ...
        var before = jdbc.queryForMap("SELECT * FROM order_outbox WHERE event_id=?", message.eventId());
        ...
        assertThat(jdbc.queryForMap("SELECT * FROM order_outbox WHERE event_id=?", message.eventId())).isEqualTo(before);
    ...
    @Test void repeatedFailureIsDeferredSoAnotherDueRowCanProgress() {
        ...
        for (int attempt = 1; attempt <= 3; attempt++) {
            ...
            assertThat(jdbc.queryForObject("SELECT next_attempt_at > clock_timestamp() FROM order_outbox WHERE event_id=?",
            ...
            jdbc.update("UPDATE order_outbox SET next_attempt_at=clock_timestamp()-interval '1 second' WHERE event_id=?",
                    failing.eventId());
        }
        assertThat(store.stats().failedPendingCount()).isEqualTo(1);
        assertThat(jdbc.queryForObject("SELECT last_error_code FROM order_outbox WHERE event_id=?",
    ...
    @Test void attemptCountSaturatesInsteadOfOverflowing() {
        OutboxMessage message = seed();
        jdbc.update("UPDATE order_outbox SET attempt_count=? WHERE event_id=?", Long.MAX_VALUE - 1, message.eventId());
    ...
    @Test void statsIncludeLiveExpiredAndDeferredClaimsButExcludeDelivered() {
        ...
        jdbc.update("UPDATE order_outbox SET next_attempt_at=clock_timestamp()+interval '1 hour', "
                + "last_error_code='SEND_FAILED' WHERE event_id=?", deferred.eventId());
        OutboxMessage delivered = seed();
        jdbc.update("UPDATE order_outbox SET delivered_at=clock_timestamp(), last_error_code='SEND_FAILED', "
                + "created_at=clock_timestamp()-interval '1 day' WHERE event_id=?", delivered.eventId());
        jdbc.update("UPDATE order_outbox SET created_at=clock_timestamp()-interval '2 minutes' WHERE event_id=?",
                live.eventId());
    ...
    @Test void operationsCommitIndependentlyOfCallerRollbackAndLeaveNoTransactionOpen() {
        ...
        outer.executeWithoutResult(tx -> {
            jdbc.update("UPDATE order_outbox SET delivered_at=clock_timestamp() WHERE event_id=?", message.eventId());
            assertThat(store.stats().pendingCount()).isEqualTo(1);
            tx.setRollbackOnly();
        });
        jdbc.update("UPDATE order_outbox SET next_attempt_at=clock_timestamp() WHERE event_id=?", message.eventId());
    ...
    private void expireLease(UUID id) {
        ...
        jdbc.update("UPDATE order_outbox SET lease_until=clock_timestamp()-interval '1 second' WHERE event_id=?", id);
    }

    private void assertClearedLease(UUID id) {
        var row = jdbc.queryForMap("SELECT lease_token, lease_until FROM order_outbox WHERE event_id=?", id);
    ...
    private OutboxMessage seed() {
        ...
        jdbc.update("INSERT INTO orders (id, customer_id, status, currency, placed_at) "
                + "VALUES (?, 'delivery-test', 'PENDING_INVENTORY', 'EUR', clock_timestamp())", orderId);
        String payload = " {\"eventId\":\"" + eventId + "\",\"orderId\":\"" + orderId
                + "\",\"eventType\":\"OrderPlaced\",\"schemaVersion\":1} \n";
        jdbc.update("INSERT INTO order_outbox (event_id, order_id, event_type, schema_version, topic, message_key, payload) "
                + "VALUES (?, ?, 'OrderPlaced', 1, 'commerce.orders.v1', ?, ?)", eventId, orderId, orderId.toString(), payload);
        return new OutboxMessage(eventId, orderId, "commerce.orders.v1", orderId.toString(), payload);
    }
}
