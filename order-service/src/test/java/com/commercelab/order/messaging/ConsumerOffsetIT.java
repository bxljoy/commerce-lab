...
@SpringBootTest(properties = {"order.outbox.enabled=false", "order.events.enabled=false"})
...
class ConsumerOffsetIT {
    ...
    @Autowired InventoryResultListener listener;
    ...
    @BeforeEach void setup() throws Exception {
        var registered = listeners.getListenerContainer("order-workflow");
        ...
        jdbc.execute("TRUNCATE order_event_inbox, order_outbox, order_requests, order_lines, orders CASCADE");
    ...
    @Test void databaseSerializationFailureRollsBackThenReplaysTheSameOffset() throws Exception {
        ...
        jdbc.execute("CREATE TRIGGER task5_transient BEFORE INSERT ON order_event_inbox "
        ...
        try {
            ...
            jdbc.execute("DROP TRIGGER task5_transient ON order_event_inbox");
    ...
    private int inbox(UUID event) {
        return jdbc.queryForObject("SELECT count(*) FROM order_event_inbox WHERE event_id=?", Integer.class, event);
    }
    ...
    private void assertState(Event event) {
        assertThat(jdbc.queryForObject("SELECT status FROM orders WHERE id=?", String.class, event.order()))
                .isEqualTo("CONFIRMED");
        assertThat(jdbc.queryForObject("SELECT version FROM orders WHERE id=?", Long.class, event.order())).isEqualTo(1);
