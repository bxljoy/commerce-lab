...
class WorkflowConsumerLifecycleTest {
    ...
    WorkflowConsumerLifecycle lifecycle() {
        when(registry.getListenerContainer("inventory-workflow")).thenReturn(container);
        doReturn(timer).when(worker).scheduleWithFixedDelay(any(Runnable.class), eq(0L), eq(5L), eq(TimeUnit.SECONDS));
        return new WorkflowConsumerLifecycle(registry, "inventory-workflow", worker);
    }
    ...
    @Test void realConcurrentContainerCleansFirstChildWhenSecondConstructionFails() throws Exception {
        ...
        when(registry.getListenerContainer("inventory-workflow")).thenReturn(parent);
        doReturn(timer).when(worker).scheduleWithFixedDelay(any(), anyLong(), anyLong(), any());
        try (var lifecycle = new WorkflowConsumerLifecycle(registry, "inventory-workflow", worker)) {
            lifecycle.start();
            var retry = attempt();
            retry.run();
    ...
    @Test void shutdownDuringInFlightStartStopsLateContainerAndTerminatesWorker() throws Exception {
        ...
        var lifecycle = new WorkflowConsumerLifecycle(registry, "inventory-workflow", executor);
        try {
            lifecycle.start();
