...
cleanup() {
  ...
  for kind in container network volume; do
    ...
    if ! remaining="$(docker "$kind" ls "$list_flag" --filter "label=com.docker.compose.project=$COMPOSE_PROJECT_NAME")"; then
      result=1
...
service_url() {
  printf 'http://%s' "$(docker compose port "$1" "$2")"
}

wait_for_health() {
  local url=$1 deadline=$((SECONDS + 90))
  while ((SECONDS < deadline)); do
    if curl --connect-timeout 1 --max-time 2 -fsS "$url/actuator/health" >/dev/null 2>&1; then
      return
    fi
    sleep 1
  done
  echo "Health deadline exceeded: $url" >&2
  return 1
}
